...
# e.g $python monaWS1S.py >> output.txt
# the output will be directed to the output.txt
# the output format is as follows (3 columns):
# nameOfTheFile ExperimentTimeInMiliSecond Satisfiablity
...
def main():
    ...
    dir = 'C:\github\automatark\ws1s'
    #dir = os.path.join(dir, 'generated-formulae') # this joins the current directory and the directory to MonaFiles
    ...
    for root, dirs, files in os.walk(dir):
        for fname in files:
            ...
            #fullName = 'generated-formulae/'+fname
...
def callProcess(command, filename):
    ...
    totalTime = 5000 #default totaltime is timeout on 5000ms
    #it will be refreshed if the experiment does not exceed time limit
    output = proc.stdout
    satisfiability = satisfiability = 'false'
    if output is None:
